<div>
    <section class="section custom-section">
        ...
        <div class="section-body">
            <div class="card">
                <div class="tw-tracking-wider tw-text-[#34395e] tw-ml-6 tw-mt-6 tw-mb-5 lg:tw-mb-1 tw-text-base tw-font-semibold tw-flex">
                    <h3>Detail Soal</h3>
                    <div class="ml-auto mr-3">
                        <span class="tw-bg-orange-50 tw-rounded-md tw-text-sm tw-px-3 tw-py-1.5 tw-text-orange-700 tw-tracking-wider">
                            ...
                            <table>
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="text-right tw-font-semibold tw-text-gray-800">{{ $bank_soal->kode_bank }}</td>
                                    </tr>
                                    <tr>
                                        <td>Mata Pelajaran</td>
                                        <td class="text-right tw-font-semibold tw-text-gray-800">{{ $bank_soal->nama_mapel }}</td>
                                    </tr>
                                    <tr>
                                        <td>Guru</td>
                                        <td class="text-right tw-font-semibold tw-text-gray-800">{{ $bank_soal->nama_guru }}</td>
                                    </tr>
                                    <tr>
                                        <td>Kelas</td>
                                        <td class="text-right tw-font-semibold tw-text-gray-800">
                                            @foreach($bank_soal->getKelas() as $class)
                                                <span class="tw-bg-purple-50 tw-rounded-full tw-px-3 tw-py-1 tw-text-purple-700 tw-tracking-wide">{{ $class->kode_kelas }}</span>
                                            @endforeach
                                        </td>
                                    </tr>
                                </tbody>
                            ...
                            <table>
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="text-right tw-font-semibold tw-text-gray-800">{{ $bank_soal->total_soal }}</td>
                                    </tr>
                                    <tr>
                                        <td>Total Soal Dibuat</td>
                                        <td class="text-right tw-font-semibold tw-text-gray-800">{{ $seluruh_total_dibuat }}</td>
                                    </tr>
                                    <tr>
                                        <td>Total Ditampilkan</td>
                                        <td class="text-right tw-font-semibold tw-text-gray-800">0</td>
                                    </tr>
                                    <tr>
                                        <td>Ket.</td>
                                        <td class="text-right tw-font-semibold tw-text-gray-800">
                                            <span class="tw-bg-orange-50 tw-rounded-md tw-px-3 tw-py-1.5 tw-text-orange-700">BELUM SELESAI</span>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
            ...
            <div class="card">
                <div class="card-body px-4">
                    <div>
                        <button wire:click="jenis('1')" class="btn tw-text-sm {{ $selectedJenis === '1' ? 'btn-primary' : 'btn-dropdown' }}">
                            Pilihan Ganda
                        </button>
                        <button wire:click="jenis('2')" class="btn tw-text-sm {{ $selectedJenis === '2' ? 'btn-primary' : 'btn-dropdown' }}">
                            Pilihan Ganda Kompleks
                        </button>
                        <button wire:click="jenis('3')" class="btn tw-text-sm {{ $selectedJenis === '3' ? 'btn-primary' : 'btn-dropdown' }}">
                            Menjodohkan
                        </button>
                        <button wire:click="jenis('4')" class="btn tw-text-sm {{ $selectedJenis === '4' ? 'btn-primary' : 'btn-dropdown' }}">
                            Isian Kompleks
                        </button>
                        <button wire:click="jenis('5')" class="btn tw-text-sm {{ $selectedJenis === '5' ? 'btn-primary' : 'btn-dropdown' }}">
                            Essai/Uraian
                        </button>
                    </div>
                    <div>
                        <table class="mt-3">
                            ...
                            <tbody>
                                <tr class="text-center tw-font-semibold">
                                    <td rowspan="4">
                                        {{ 
                                            $selectedJenis == "1" ? "Pilihan Ganda" :
                                            ($selectedJenis == "2" ? "Pilihan Ganda Kompleks" :
                                            ($selectedJenis == "3" ? "Menjodohkan" :
                                            ($selectedJenis == "4" ? "Isian Kompleks" :
                                            ($selectedJenis == "5" ? "Essai / Uraian" : ""))))
                                        }}
                                    </td>
                                </tr>
                                <tr>
                ...
                <div class="card-body px-4">
                    <div class="tw-flex">
                        <p class=" tw-font-semibold tw-text-gray-700 tw-tracking-wider">
                            {{ 
                                $selectedJenis == "1" ? "A. Soal Pilihan Ganda (Opsi: $bank_soal->opsi Soal) " :
                                ($selectedJenis == "2" ? "B. Pilihan Ganda Kompleks" :
                                ($selectedJenis == "3" ? "C. Menjodohkan" :
                                ($selectedJenis == "4" ? "D. Isian Kompleks" :
                                ($selectedJenis == "5" ? "E. Essai / Uraian" : ""))))
                            }}
                        </p>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-body px-4">
                        <div class="tw-flex tw-relative">
                            ...
                            @if ($row->tampilkan == "1")
                            <button wire:click.prevent="status({{ $row->id }}, '0')" class="ml-auto tw-absolute tw-top-0 tw-right-0">
                                <label class="switch">
                            ...
                            @else
                            <button wire:click.prevent="status({{ $row->id }}, '1')" class="ml-auto tw-absolute tw-top-0 tw-right-0">
                                <label class="switch">
                                    <input type="checkbox">
                                    <span class="slider round"></span>
                                </label>
                        ...
                        <ul class="ml-3 my-2 tw-flex tw-flex-wrap">
                            @php
                            $options = ['A' => 'opsi_a', 'B' => 'opsi_b', 'C' => 'opsi_c', 'D' => 'opsi_d', 'E' => 'opsi_e'];
                            ...
                            @if ($isLongOption)
                                <div class="tw-w-full">
                                    @foreach ($options as $key => $option)
                                        @if (!empty($row->$option))
                                            <li class="tw-flex tw-ml-3">{{ $key }}. &nbsp;{!! $row->$option !!}</li>
                                        @endif
                                    @endforeach
                                </div>
                            @else
                        ...
                        <div class="tw-flex mt-2">
                            ...
                            <div class="ml-auto tw-flex">
                                <button wire:click.prevent="edit({{ $row->id }})" class="btn btn-primary mr-1" data-toggle="modal" data-target="#formDataModal">
                ...
                <form>
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            </div> @error('soal') <small class='text-danger'>{{ $message }}</small> @enderror
                        </div>
                        ...
                        <div class="form-group mt-3">
                            ...
                            </div> @error('jawaban') <small class='text-danger'>{{ $message }}</small> @enderror
                        </div>
                    </div>
