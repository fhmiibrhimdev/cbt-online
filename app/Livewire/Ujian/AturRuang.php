...
class AturRuang extends Component
{
    ...
    public function mount($id_kelas = "")
    {
        ...
        if ($id_kelas == "") {
            ...
            $this->siswas = Kelas::select('siswa.id', 'kelas.kode_kelas', 'siswa.nama_siswa', 'sesi_siswa.id_ruang', 'sesi_siswa.id_sesi')
    ...
    public function updatedData()
    {
        if ($this->id_kelas == "") {
            ...
            $this->siswas = Kelas::select('siswa.id', 'kelas.kode_kelas', 'siswa.nama_siswa')
                        ->join('kelas_detail', 'kelas_detail.id_kelas', 'kelas.id')
                        ->leftJoin('siswa', 'siswa.id', 'kelas_detail.id_siswa')
                        ->where('kelas.id', $this->id_kelas)
                        ->get();
    ...
    {
        $kelass = Kelas::select('id', 'kode_kelas')->get();
    ...
    public function store()
    {
        ...
        foreach ($this->siswas as $siswa) {
            // dd($this->data[$siswa->id]['id_ruang']);
            SesiSiswa::create([
